...
export default function ConnectFour() {
  ...
  return (
    <PageShell label="games/connect-four" title="Connect Four">
          ...
          <div key={r} className="flex gap-1">
            {row.map((c, ci) => (
              <button
                key={ci}
                onClick={() => drop(ci)}
                className="w-10 h-10 sm:w-12 sm:h-12 bg-background rounded-full border border-border flex items-center justify-center"
              >
                <span
                  className={`w-7 h-7 sm:w-9 sm:h-9 rounded-full ${
                    c === 1 ? "bg-primary" : c === 2 ? "bg-accent" : ""
                  }`}
                />
              </button>
            ))}
      ...
      <div className="mt-6">
        <button
          onClick={reset}
          className="font-mono text-xs uppercase tracking-wider text-primary border border-primary px-4 py-2 hover:bg-primary hover:text-primary-foreground transition-colors"
        >
          [ reset ]
        </button>
      </div>
